...
export default function CurriculumLesson() {
  ...
  useEffect(() => {
    ...
    dispatch(getTeacherLessonsById(id));
          ...
          <div className="w-full flex items-center pt-4 sm:pt-6">
            <div className="w-fit max-w-full rounded-full border border-black bg-white p-1 font-medium text-black flex items-center shadow-xs">
              <button
                ...
                className={`whitespace-nowrap rounded-full px-5 py-2 text-sm transition-colors duration-200 cursor-pointer ${activeTab === "lesson"
              ...
              <button
                ...
                className={`whitespace-nowrap rounded-full px-5 py-2 text-sm transition-colors duration-200 cursor-pointer ${activeTab === "reviews"
                    ? "bg-primary text-white shadow-sm"
                    : "text-black hover:bg-gray-100"
                  }`}
              >
          ...
          {activeTab === "reviews" ? (
            ...
            <div className="w-full">
              ...
              <div className="w-full mt-[32px]">
                ...
                {/* This lesson is also part of a curriculum link (under title) */}
                {currilesson?.length > 0 && (
                  <div className="flex items-center gap-2 text-sm text-gray-700 mt-2">
                    <Info size={16} className="text-gray-500 shrink-0" />
                    <span>
                      This lesson is also part of a curriculum:{" "}
                      {currilesson.map((item, index) => (
                        <Link
                          to={`/curriculum-booking/${item?.curriculumInfo?._id}`}
                          key={index}
                          className="text-primary font-medium hover:underline"
                        >
                          {item?.curriculumInfo?.title}
                          {index < currilesson.length - 1 ? ", " : ""}
                        </Link>
                      ))}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6 mt-[32px] h-fit w-full items-start">
                ...
                <div className="w-full lg:max-h-[calc(100vh-190px)] lg:overflow-y-auto custom-scrollbar pr-1">
                  <ImageGallery images={lesson?.images || []} layout="stack" />
                </div>

                {/* COLUMN 3: Description & How It Works (Bubble) */}
                <div className="w-full bg-[#E9EAEE] rounded-[24px] p-5 sm:p-6 shadow-none flex flex-col space-y-6 h-fit lg:max-h-[calc(100vh-190px)] lg:overflow-y-auto overflow-x-hidden custom-scrollbar">
                  {/* Description Section */}
                  <div className="w-full">
                    <h2 className="text-base sm:text-lg md:text-xl font-semibold mb-3 text-black">
                      Description
                    </h2>
                    <div className="text-gray-700 leading-relaxed text-sm break-words [overflow-wrap:anywhere]">
                      {lesson?.description ? (
