...
export default function LessonPayment() {
  ...
  const handleContinue = () => {
    if (loading) return;
    if (!formData.firstName?.trim() || !formData.lastName?.trim() || !formData.country) {
      toast.error("Please fill in all student details");
      return;
    }
    if (!bookingData?.newDate) {
          ...
          {!clientSecret && (
            <>
              ...
              <button
                ...
                disabled={loading || !formData.firstName?.trim() || !formData.lastName?.trim() || !formData.country}
                className="w-full bg-primary text-white rounded-md py-3 mt-6 transition hover:bg-primary/95 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
